...
def vincular_proveedor_producto(user_id, supplier_id, product_id):
    ...
    try:
        ...
        print(f"ERROR en vincular proveedor-producto: {str(e)}")
        return jsonify({"error": str(e)}), 500
...
@app.route('/usuario/<int:user_id>/articulos/<int:product_id>/proveedores', methods=['GET'])
...
@app.route('/usuario/<int:user_id>/distribuidores/<int:supplier_id>/productos', methods=['GET'])
